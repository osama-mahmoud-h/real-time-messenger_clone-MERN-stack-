...
useEffect(async () => {
  ...
  socket.current?.on("connect", () => {
    ...
    socket.current?.on("getFriends_success", (frnds) => {
      console.log("getFriends_success event", frnds);
      setAllFriends(frnds);
    ...
    socket.current?.on("Messages-Get-Success", (data) => {
      console.log(
        "message recieve curr ",
        currentFriendRef.current._id,
        " ",
        currentFriendRef.current.userName,
        " ",
      ...
      if (
        ...
        console.log("setScoketConversation: ", data.message);
    ...
    socket.current?.on("greeting",()=>{
        console.log("webSocket greeeeeting-----------------------------")
    });
    

    console.log("get friends: myId: ", userInfo.id);
    //socket.current.emit
    // Additional actions to perform on successful connection
  });
    ...
    useEffect(() => {
        if (allFriends && allFriends.length > 0) {
           ...
           // setAllFriends(friends)
            ...
            console.log("mongodb convers .. :",scoketConversation.length)
        }
            ...
            <div className="left-side">
                <div className="top">
                    <div className="image-name">
                        <div className="image">
                        <div className="initials-placeholder">{userInfo.userName.substring(0, 2).toUpperCase()}</div>
